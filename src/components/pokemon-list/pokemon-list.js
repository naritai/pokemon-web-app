...
export default class PokemonList extends Component {
  ...
  renderItems() {
    ...
    return this.search(pokemonList, term)
      .map(({ name }) => 
        (<li key={ name }
             onClick={ () => this.onItemSelected(name) }> 
        <PokemonItem name={ name } /> </li> ));
  }
  ...
  search(items, label) {
    ...
    return items.filter(item => 
      item.name.indexOf(label) !== -1 );
  }

  render() {
    const { pokemonList, loading, error } = this.state;
    const spinner = loading ? <LoaderIndicator /> : null;
    const errorMessage = error ? <span>Something went wrong, sorry.</span> : null;
